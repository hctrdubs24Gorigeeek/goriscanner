...
/*The code was taken from https://pub.dev/packages/qr_scanner_overlay
and it was modified to comply with the design of the app*/
class InvertedClipper extends CustomClipper<Path> {
  late Size scanArea;
  late double borderLength;
  InvertedClipper({required this.scanArea, this.borderLength = 20.0});
  ...
  Path getClip(Size size) {
    return Path()
      ...
      ..addRRect(RRect.fromRectAndRadius(
          ...
          Radius.circular(borderLength - 4)))
...
class QRScannerOverlayCustom extends StatelessWidget {
    ...
    return Stack(children: [
      ...
      Align(
        ...
        child: CustomPaint(
          ...
          child: SizedBox(
            width: (_scanArea?.width ?? standardScanArea.width) + 5.0,
            height: (_scanArea?.height ?? standardScanArea.height) + 5.0,
          ),
        ),
      ),
    ]);
...
class BorderPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final rrect = RRect.fromRectAndRadius(rect, Radius.circular(1));
